...
class Main:
    def __init__(self):
        ...
        self.santas = [[0, 0, 0]] + [list(map(int, input().split())) for _ in range(self.p)]
    ...
    def find_santa(self):
        ...
        for i in range(1, self.p + 1):  # 가장 가까운 산타 찾기
            if not self.alive[i]:
                continue
            _, sy, sx = self.santas[i]
            distance = self.cal_distance(self.y, self.x, sy, sx)
            if distance <= min_distance:
                min_distance = distance
                distances.append((i, sy, sx))
    ...
    def move_rudolph(self, turn):
        ...
        if self.y == sy and self.x == sx:  # 이동한 곳에 산타가 있다면
    ...
    def back_santa(self, santa, sy, sx, dy, dx, toward):
        dq = deque([(santa, sy, sx, dy, dx, toward)])

        while dq:
            santa, sy, sx, dy, dx, toward = dq.popleft()
            my, mx = sy + (dy * toward), sx + (dx * toward)
            if 1 <= my <= self.n and 1 <= mx <= self.n:
                if self.board[my][mx] == 0:  # 빈칸이면
                    self.santas[santa] = [santa, my, mx]
                    self.board[my][mx] = santa
                    return
                else:
                    dq.append((self.board[my][mx], my, mx, dy, dx, 1))
                    self.santas[santa] = [santa, my, mx]
                    self.board[my][mx] = santa
            else:  # 보드를 벗어나면
    ...
    def init_board(self):
        for i in range(1, self.p + 1):  # 산타 위치
            _, y, x = self.santas[i]
            self.board[y][x] = i
